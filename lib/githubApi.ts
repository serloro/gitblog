...
class GitHubApiService {
  ...
  // NEW: Extract baseurl from repository URL
  private getBaseurlFromRepo(): string {
    // If the repo name is the same as owner.github.io, baseurl should be empty
    if (this.repo === `${this.owner}.github.io`) {
      return '';
    }
    // Otherwise, baseurl should be the repo name with leading slash
    return `/${this.repo}`;
  }
...
- **Sitio web:** [Ver blog en vivo](https://${this.owner}.github.io${this.getBaseurlFromRepo()})
...
`;
  ...
  private getDefaultJekyllConfig(): string {
    const baseurl = this.getBaseurlFromRepo();
...
`;
  ...
  generateJekyllConfig(config: {
    ...
    // CRITICAL: Use auto-calculated baseurl instead of user input
    const baseurl = this.getBaseurlFromRepo();
